...
The currently live `truthtrace.ai` service is now attributable to an authenticated Lovable workspace, an exact Lovable project, an exact private GitHub repository and branch, and an exact live commit. That materially improves the production record. Publication is still blocked because Lovable did not expose an immutable deployment ID, the audited PR #4 tree is not the tree currently held by either verified Lovable project, no exact candidate cutover command has been proven, and no deployment rollback command or immutable prior deployment has been identified or rehearsed.
...
| Label            | Meaning                                                                                                 |
| ---------------- | ------------------------------------------------------------------------------------------------------- |
| `VERIFIED`       | Directly established from authoritative DNS, authenticated provider/GitHub data, or current live output |
...
| --------------------------- | ---------------- | -------------------------------------------------------------------------------------------------- |
| Observation window          | `VERIFIED`       | 2026-07-15 01:29–01:40 UTC (2026-07-14 20:29–20:40 America/Chicago)                                |
...
| Approved apex/`www` normalization | `BLOCKED`  | Apex and `www` both serve `200`; neither canonical host redirects to the other. A primary-domain decision must be applied and verified before cutover.                                                                                                                    |
...
| Exact candidate provider preview deployment | `BLOCKED`  | No isolated provider deployment tied to the exact PR #4/candidate SHA has been created. The existing `truth-trace-forge.lovable.app` publication is not the audited branch.                                                                                                        |
...
| P0-PI-5 | `BLOCKED` | Identify authenticated Hostinger DNS primary/backup operators, export the complete before-state, choose apex or `www` as primary, and prove the no-change or rollback procedure                                 |
| P0-PI-6 | `BLOCKED` | Ensure PR #4's final implementation SHA is the only runtime difference entering the chosen provider project; the current `truthtrace-website` and `truth-trace-forge` provider snapshots are not that candidate |
| P0-PI-7 | `BLOCKED` | Remove or explicitly approve legacy Google/Lovable analytics behavior for the browse-only replacement, with a live network/cookie verification on the exact staging deployment                                  |
